...
const projects = [
  {
    title: 'UnlockHub',
    type: 'SaaS Dashboard',
    description: 'A multi-page account-management dashboard built around authentication, access states, tools, activity tracking, transactions and premium experiences.',
    stack: ['React', 'TypeScript', 'Tailwind', 'Zustand'],
    href: 'https://github.com/TEEKAY-alt403',
  },
  {
    title: 'E-commerce Store',
    type: 'Frontend Application',
    description: 'A responsive storefront concept focused on product discovery, routing, shopping-cart state, forms, authentication and a clean customer journey.',
  ...
  },
  {
    ...
    type: 'Personal Brand',
    description: 'A recruiter-focused portfolio designed to communicate frontend ability, practical project experience and a clear developer identity.',
    stack: ['React', 'TypeScript', 'Tailwind'],
    href: 'https://github.com/TEEKAY-alt403/portfolio.teekay',
  },
]
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#08090c] text-zinc-100">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/5 bg-[#08090c]/80 backdrop-blur-xl">
        <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 lg:px-8" aria-label="Main navigation">
          <a href="#home" className="text-lg font-black tracking-tight" onClick={closeMenu}>T<span className="text-indigo-400">.</span></a>
      ...
      <main>
        <section id="home" className="grid-bg relative flex min-h-screen items-center px-5 pt-24 lg:px-8">
          ...
          <div className="mx-auto grid w-full max-w-6xl gap-12 py-20 lg:grid-cols-[1.2fr_.8fr] lg:items-center">
            <div>
              <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[.03] px-4 py-2 text-sm text-zinc-300"><span className="h-2 w-2 rounded-full bg-emerald-400" /> Open to opportunities</div>
              <p className="mb-4 text-sm font-semibold uppercase tracking-[.25em] text-indigo-400">Frontend Developer</p>
              <h1 className="max-w-4xl text-5xl font-black leading-[.95] tracking-[-.05em] sm:text-7xl lg:text-8xl">I build digital experiences that <span className="text-zinc-500">feel right.</span></h1>
              <p className="mt-8 max-w-2xl text-lg leading-8 text-zinc-400">I'm Teekay, a Computer Science student and frontend developer focused on turning ideas into responsive, accessible and polished web interfaces.</p>
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="grid gap-10 lg:grid-cols-2">
              ...
              <div className="space-y-5 text-lg leading-8 text-zinc-400">
                <p>I enjoy the space where design and engineering meet — taking a rough idea, understanding the user behind it, and building an interface that is useful, clear and memorable.</p>
                <p>My Computer Science background gives me a strong technical foundation, while hands-on projects keep me learning modern frontend patterns, deployment workflows and practical product development.</p>
                <p>I'm especially interested in opportunities where I can contribute to a real product, learn from experienced developers and keep improving as an engineer.</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="skills" className="border-t border-white/5 bg-white/[.015] px-5 py-28 lg:px-8">
          <div className="mx-auto max-w-6xl">
            ...
            <h2 className="mb-10 text-4xl font-bold tracking-tight sm:text-5xl">A practical frontend toolkit.</h2>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{skills.map((skill) => <div key={skill.name} className="rounded-2xl border border-white/8 bg-[#0d0f14] p-5 transition hover:-translate-y-1 hover:border-indigo-400/30"><p className="text-xs uppercase tracking-[.18em] text-zinc-600">{skill.group}</p><p className="mt-2 font-medium text-zinc-100">{skill.name}</p></div>)}</div>
          </div>
        </section>

        <section id="projects" className="border-t border-white/5 px-5 py-28 lg:px-8">
          <div className="mx-auto max-w-6xl">
            <div className="mb-12 flex items-end justify-between gap-6"><div><p className="mb-3 text-sm font-semibold uppercase tracking-[.25em] text-indigo-400">03 — Projects</p><h2 className="text-4xl font-bold tracking-tight sm:text-5xl">Selected work.</h2></div><Sparkles className="hidden text-indigo-400 sm:block" /></div>
            <div className="grid gap-5 lg:grid-cols-3">
              {projects.map((project, index) => <article key={project.title} className="group flex min-h-[410px] flex-col rounded-3xl border border-white/8 bg-[#0d0f14] p-7 transition duration-300 hover:-translate-y-2 hover:border-indigo-400/30">
        ...
        <section id="contact" className="border-t border-white/5 px-5 py-28 lg:px-8">
          <div className="mx-auto max-w-6xl rounded-[2rem] border border-white/8 bg-gradient-to-br from-indigo-500/10 to-transparent p-8 sm:p-14">
            <p className="mb-3 text-sm font-semibold uppercase tracking-[.25em] text-indigo-400">05 — Contact</p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <a href="https://github.com/TEEKAY-alt403" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 font-semibold text-black transition hover:bg-zinc-200"><Github size={17} /> GitHub</a>
              <a href="#" className="inline-flex items-center gap-2 rounded-full border border-white/10 px-6 py-3 font-semibold text-zinc-200 transition hover:border-white/25 hover:bg-white/5"><Mail size={17} /> Contact me</a>
            </div>
          </div>
        </section>
      </main>

      <footer className="border-t border-white/5 px-5 py-8 lg:px-8"><div className="mx-auto flex max-w-6xl flex-col justify-between gap-5 text-sm text-zinc-500 sm:flex-row sm:items-center"><p>© {new Date().getFullYear()} Teekay. Built with React.</p><a href="https://github.com/TEEKAY-alt403" target="_blank" rel="noreferrer" className="transition hover:text-white"><Github size={19} /></a></div></div></footer>
    </div>
  )
}
