...
import { ArrowDown, ArrowUpRight, BriefcaseBusiness, Code2, Github, GraduationCap, Mail, Menu, Sparkles, X } from 'lucide-react'
...
const projects = [
  { title: 'UnlockHub', type: 'SaaS Dashboard', description: 'A multi-page account-management dashboard built around authentication, access states, tools, activity tracking, transactions and premium experiences.', stack: ['React', 'TypeScript', 'Tailwind', 'Zustand'], href: 'https://github.com/TEEKAY-alt403' },
  { title: 'E-commerce Store', type: 'Frontend Application', description: 'A responsive storefront concept focused on product discovery, routing, shopping-cart state, forms, authentication and a clean customer journey.', stack: ['React', 'JavaScript', 'CSS'], href: 'https://github.com/TEEKAY-alt403' },
  { title: 'Portfolio Website', type: 'Personal Brand', description: 'A recruiter-focused portfolio designed to communicate frontend ability, practical project experience and a clear developer identity.', stack: ['React', 'TypeScript', 'Tailwind'], href: 'https://github.com/TEEKAY-alt403/portfolio.teekay' },
]
      ...
      <main>
        <section id="home" className="grid-bg relative flex min-h-screen items-center px-5 pt-24 lg:px-8">
          ...
          <div className="mx-auto grid w-full max-w-6xl gap-12 py-20 lg:grid-cols-[1.2fr_.8fr] lg:items-center">
            <div><div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[.03] px-4 py-2 text-sm text-zinc-300"><span className="h-2 w-2 rounded-full bg-emerald-400" /> Open to opportunities</div><p className="mb-4 text-sm font-semibold uppercase tracking-[.25em] text-indigo-400">Frontend Developer</p><h1 className="max-w-4xl text-5xl font-black leading-[.95] tracking-[-.05em] sm:text-7xl lg:text-8xl">I build digital experiences that <span className="text-zinc-500">feel right.</span></h1><p className="mt-8 max-w-2xl text-lg leading-8 text-zinc-400">I'm Teekay, a Computer Science student and frontend developer focused on turning ideas into responsive, accessible and polished web interfaces.</p><div className="mt-10 flex flex-wrap gap-4"><a href="#projects" className="inline-flex items-center gap-2 rounded-full bg-indigo-500 px-6 py-3 font-semibold text-white transition hover:bg-indigo-400">View my work <ArrowDown size={17} /></a><a href="#contact" className="inline-flex items-center gap-2 rounded-full border border-white/10 px-6 py-3 font-semibold text-zinc-200 transition hover:border-white/25 hover:bg-white/5">Get in touch <ArrowUpRight size={17} /></a></div></div>
